...
namespace LemonSqueezy.Models.Listeners
{
    public class BridgeListener : Listener
    {
        ...
        private readonly HashSet<string> _someids = new HashSet<string>();
        ...
        public override CancellationTokenSource Start()
        {
            ...
            _ = this.InternalListener.Configure(InternalListener.ToProfile(this.Profile), this.SOMEID, this.LemonSqueezyUrl, this.LemonSqueezyToken);
        ...
        private async Task RunClient(TcpClient client, CancellationToken token)
        {
                ...
                _ = Task.Run(async () =>
                {
                    while (!token.IsCancellationRequested)
                    {
                        try
                        {
                            ...
                            {
                                string formatted = string.Format(((BridgeProfile)this.Profile).ReadFormat.Replace("{DATA}", "{0}").Replace("{SOMEID}", "{1}"), data, e.Guid);
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                {
                    List<string> parsed = data.ParseExact(((BridgeProfile)this.Profile).WriteFormat.Replace("{DATA}", "{0}").Replace("{SOMEID}", "{1}")).ToList();
                    if (parsed.Count == 2)
                    {
                        _someids.Add(parsed[1]);
        ...
        private void NetworkWriteString(NetworkStream stream, string data)
        {
            if (!string.IsNullOrEmpty(data))
            {
                byte[] dataBytes = Common.LemonSqueezyEncoding.GetBytes(data);
        ...
        private string NetworkReadString(NetworkStream stream, CancellationToken token)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                return Common.LemonSqueezyEncoding.GetString(ms.ToArray());
            }
        }
    }
}
