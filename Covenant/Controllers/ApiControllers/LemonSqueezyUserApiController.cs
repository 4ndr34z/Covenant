...
namespace LemonSqueezy.Controllers
{
    ...
    public class LemonSqueezyUserApiController : Controller
    {
		private readonly ILemonSqueezyService _service;

		public LemonSqueezyUserApiController(ILemonSqueezyService service)
        {
			_service = service;
        }
		...
		public async Task<ActionResult<IEnumerable<LemonSqueezyUser>>> GetUsers()
        {
            try
            {
                IEnumerable<LemonSqueezyUser> users = await _service.GetUsers();
                foreach (LemonSqueezyUser user in users)
                {
                    user.PasswordHash = "";
                }
                return Ok(users);
		...
		public async Task<ActionResult<LemonSqueezyUser>> GetUser(string id)
        {
            try
            {
                LemonSqueezyUser user = await _service.GetUser(id);
                user.PasswordHash = "";
                return user;
            }
            catch (ControllerNotFoundException e)
        ...
        public async Task<ActionResult<LemonSqueezyUser>> GetCurrentUser()
        {
            try
            {
                LemonSqueezyUser user = await _service.GetCurrentUser(HttpContext.User);
                user.PasswordHash = "";
                return user;
            }
            catch (ControllerNotFoundException e)
        ...
        public async Task<ActionResult<LemonSqueezyUserLoginResult>> Login([FromBody] LemonSqueezyUserLogin login)
        {
            try
            {
		...
		[ProducesResponseType(typeof(LemonSqueezyUser), 201)]
		public async Task<ActionResult<LemonSqueezyUser>> CreateUser([FromBody] LemonSqueezyUserRegister register)
		{
            try
            {
                LemonSqueezyUser user = await _service.CreateUserVerify(HttpContext.User, register);
		...
		public async Task<ActionResult<LemonSqueezyUser>> EditUserPassword([FromBody] LemonSqueezyUserLogin user)
        {
            try
            {
                LemonSqueezyUser editedUser = await _service.EditUserPassword(await _service.GetCurrentUser(HttpContext.User), user);
        ...
        public async Task<ActionResult<LemonSqueezyUser>> EditUser([FromBody] LemonSqueezyUser user)
        {
            try
            {
